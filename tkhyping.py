...
class methode :
    ...
    def Get_last_loop(self):
        ...
        if "result" in jr:
            ...
            self.print_red("GetLastRecordingName ERROR got %s\n" % (json.dumps(jr)))
            return
        ...
        while run:
            ...
            if "result" in jr:
                ...
                self.print_red("ERROR received :", jr["error"]["message"])
                break
            else:
                self.print_red("Bad response from device")
                break
    ...
    def Meter(self, cde, meter, test=False):
        ...
        if "result" in jr and jr["result"] == True:
            ...
            self.print_red("Error trying to remove EQ bank")
        if cde in ["add", "remove"]:
            ...
            while True:
                ...
                jr = self.send_req("GetInternals", params)
                if jr is None:
                    self.print_red("Error in send_req, probably a timeout")
                    break
                elif "result" in jr:
                    res = jr["result"]
    ...
    def Temp(self):
        ...
        if jr is None:
            ...
            res = jr["result"]
            self.print_green("CPU Temperature :", res["temperature"])
        else:
            self.print_red(jr)
    ...
    def Calibrate(self):
        ...
        Start a calibration
        """
        jr = self.send_req("SetController")
        if "result" in jr:
            res = jr["result"]
            print(res)
        else:
    ...
    def GetCPU(self):
        ...
        if "result" in jr:
            res = jr["result"]
            self.print_green("CPU usage :", res["cpu"], "%")
        else:
            self.print_red(jr)
...
class GUI:
    def __init__(self):
        ...
        self.method_combobox = ttk.Combobox(self.root, textvariable=self.method_var, state="readonly")
...
dico = {
 ...
 'BypassEffect': [True, 3, 'bank_num', 'effect_num', 'bypass'],
...
def usage():
    ...
    Returns
    -------
    Send a table with the different action and options during the utilisation of the terminal
    """
...
if argc == 3:
    method = sys.argv[2]
    Methode.run_methode(method, False)
elif argc >= 4:
    if sys.argv[1] == "ble":
        mode_BLE = True
        ...
        method = sys.argv[4]
        option = sys.argv[5:]
        Methode.run_methode(method, *option)
    else:
        method = sys.argv[2]
        option = sys.argv[3:]
        Methode.run_methode(method, *option)
